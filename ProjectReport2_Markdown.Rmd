...
The spatial behavior of an animal has a wide meaning such as movement, habitat selection, home range, core area, territoriality, and migration. It can be influenced by decisions relating to foraging, movement, avoidance, resting, territorial activity, mating, and rearing young. In general, wild boar prefer broad-leaved forests with older mast species (beech, oak) while foraging, and in contrast, the coniferous forest is preferred for their resting sites as well as secure places for wallowing and sleeping (Erdtmann & Keuling, 2020). These findings pave the way toward implementing effective control strategies in the wild and animal welfare in captivity. 
In this research project, we were analyzing the resting behavior. On average the wild boar is reported to be sleeping around twelve hours per day. Our main questions were how long the wild boar sleep and where they usually choose to sleep (forest or agricultural land). 
Our research is based on the assumption that resting behavior is defined by static movement (short step lengths), but it could also be that animals are in one place for a longer period of time to clean themselves. The GPS couldn’t recognize the difference between the two, therefor we had to define the treshold which could help to rule out non-resting behavior, with the assumtion, that wild boars do not clean for several hours.  


## Material and Methods
## Study area
The range of observed area mostly include Gampelen and Ins and its surroundings. Gampelen and Ins are municipalities that lay in the canton of Bern and are mostly covered with land for agricultural purposes and forest. Area lays by the Lake Neuchâtel, which provide wild bord the source of water. From the picture bellow we can see, that movement of animals is spread across the area, but the majority of the animals stay near the lake. 
...
The data was collected with GPS collars fitted to 19 wild boars. They were programmed to attempt a GPS location fix every 15 minutes. The mean time lag between consecutive points was 17.8 minutes. The total study time lasted for 874 days from the 28th of May 2014 to the 18th of October 2016. However, not all animals were monitored at the same time and some individuals show gaps in their sampling interval (see Figure bellow). 
...
In addition, we received a dataset from the ZHAW containing habitat information about the investigates area. This dataset includes the factors forest, wet meadow and different types of crops. 
...
In a first step, we explored the calculated step lengths. The mean step length of all animals was 39 meters, the median seven meters (see Figure bellow). We can see that animals are mostly not taking long distance and are moving slower.
...
We then searched for a period when many animals were monitored at the same time and visualized the steps of these individuals. In Figure bellow the seven chosen individuals (Caroline, Fritz, Isabelle, Nicole, Rosa, Ruth, Sabine) all show a similar step pattern during the first week of January 2015, with more active and more static stretches. The step lengths were longer during the day and we can see a clear static pattern at the night and mornings.

```{r, warning = FALSE, message = FALSE, echo=FALSE}

athel_January2015 <- wildschwein_BE %>%				#Making dataframe for all animals in one week in 2015
...
The graphs shows when the 

```{r}
hour_min <- table(k12_segments_2h$hour_min) #Visualing start of segments
barplot(hour_min)

hour_max <- table(k12_segments_2h$hour_max) #Visualing end of segments
barplot(hour_max)
